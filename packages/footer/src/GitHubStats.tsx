...
const GitHubStats: React.FC = () => {
	...
	return (
		<Link href="https://ijskoud.dev/github/website" target="_blank" className="flex gap-4 hover:text-primary transition-colors">
			<p className="text-4">
				<i className="fa-solid fa-code-fork" /> <span className="font-bold">{loading ? "·" : stats?.forks_count ?? 0}</span>
			</p>
			<p className="text-4">
				<i className="fa-solid fa-star" /> <span className="font-bold">{loading ? "·" : stats?.stargazers_count ?? 0}</span>
			</p>
		</Link>
	);
};
